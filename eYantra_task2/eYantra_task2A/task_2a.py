...
# Team ID:			[ 2256 ]
# Author List:		[ Aman Raj, Shreyas Kumar ]
# Filename:			task_2a.py
...
def control_logic(sim):
	...
	"""
	##############  ADD YOUR CODE HERE  ##############
	wheelr = sim.getObjectHandle('/left_joint')
	wheell = sim.getObjectHandle('/right_joint')
	sim.setJointTargetVelocity(wheell, 2)
	sim.setJointTargetVelocity(wheelr, 2)
	while(True):
		...
		print("sensor 1: ",d1)
		if(d1 <= 0.2):
			sim.setJointTargetVelocity(wheell, 0.5)
			sim.setJointTargetVelocity(wheelr, -0.5)
			time.sleep(4)
			sim.setJointTargetVelocity(wheell, 0)
			sim.setJointTargetVelocity(wheelr, 0)
			break
	##################################################
...
def detect_distance_sensor_1(sim):
	...
	detected, distance, _, __, ___ = sim.readProximitySensor(sensor_1, 1)
	if detected < 1:
		return 20
	##################################################
	return distance
...
def detect_distance_sensor_2(sim):
	...
	detected, distance, _, __, ___ = sim.readProximitySensor(sensor_2, 1)
	if detected < 1:
		return 20
