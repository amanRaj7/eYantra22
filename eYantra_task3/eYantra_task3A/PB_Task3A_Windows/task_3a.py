...
# Team ID:			[ PB_2256 ]
# Author List:		[ Aman Raj, Naveen Rajpurohit, Aniket Chanda, Shreyas Kumar ]
# Filename:			task_3a.py
...
def paths_to_moves(paths, traffic_signal):
	...
	for i in range(1, len(paths)):
		...
		if ((paths[i][0]==paths[i-1][0]) and direction==1) or ((paths[i][1]==paths[i-1][1]) and direction==0) or ((paths[i][1]==paths[i-1][1]) and direction==2):
			...
			list_moves.append('LEFT')
			direction = 1
		elif (direction==2) and paths[i][1]<paths[i-1][1]:
			list_moves.append('LEFT')
			direction = 1
		elif (direction==2) and paths[i][1]>paths[i-1][1]:
			list_moves.append('RIGHT')
